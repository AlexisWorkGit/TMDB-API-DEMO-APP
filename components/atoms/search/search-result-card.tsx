...
export const SearchResultCard: React.FC<SearchResultCardProps> = ({ media }) => {
  ...
  return (
    <Link href={`/${media_type}/${id}`} prefetch={false}>
      <MediaCard.Root>
        ...
        <MediaCard.Content>
          {!isPerson && (
            <MediaRating average={media.vote_average} count={media.vote_count} className="mr-2" />
          )}
          <Badge className="border-foreground capitalize" variant="outline">
            {media.media_type === "tv" ? "TV Show" : media.media_type}
          </Badge>

          <MediaCard.Title className="mt-2">{isMovie ? media.title : media.name}</MediaCard.Title>

          <MediaCard.Excerpt>
            {isPerson
              ? `Known for ${media.known_for_department}`
              : format.year(isMovie ? media.release_date : media.first_air_date)}
          </MediaCard.Excerpt>
        </MediaCard.Content>
      </MediaCard.Root>
    </Link>
