...
import { ProviderLogo } from "@/components/molecules/provider";
...
export const DiscoverFilterProvider: React.FC<DiscoverFilterGenreProps> = ({
  ...
  const comboboxValue = value ? joiner(selectedProviders, "provider_name") : "Select providers...";
    ...
    <div className="space-y-2">
      <Label className="flex items-center gap-2 text-muted-foreground">
        ...
        <InfoTooltip className="w-60">
          Currently showing providers are available in your region. You can change your region in
          the settings.
        </InfoTooltip>
      </Label>
      <Popover>
  ...
  return (
    <CommandItem key={id} value={name} onSelect={() => toggleSelection(id)}>
      ...
      <span className="relative mr-2 size-4">
        <ProviderLogo image={logo} alt={name} className="rounded-md" size="w45" />
      </span>
      ...
      <CommandList>
        ...
        {selection.length > 0 && (
          <CommandGroup className="border-t bg-background">
            <CommandItem className="justify-center" onSelect={() => clearSelection()}>
              Clear selection
            </CommandItem>
          </CommandGroup>
        )}
