...
} from "@/components/molecules/discover";
...
export const DiscoverFilters: React.FC<DiscoverFiltersProps> = ({ type, genres, providers }) => {
  const { count, getFilter, setFilter, saveFilters, clearFilters } = useFilters(type);

  return (
    <Sheet>
      <SheetTrigger className={cn(buttonVariants({ variant: "outline" }))}>
        <SlidersHorizontal className="mr-2 size-4" /> Filters
        {count > 0 && <Badge className="ml-2 px-2 text-xs leading-none">{count}</Badge>}
      </SheetTrigger>
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <DiscoverFilterDate
                ...
                onChange={(value) => setFilter({ "primary_release_date.gte": value })}
              />
              ...
              <DiscoverFilterDate
                ...
                onChange={(value) => setFilter({ "primary_release_date.lte": value })}
              />
            </div>
