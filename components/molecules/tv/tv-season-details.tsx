...
import { TvEpisodeCard, TvSeasonDialog } from "@/components/molecules/tv";
...
export const TvSeasonDetails: React.FC<TvSeasonDetailsProps> = async ({ id, season, ...props }) => {
  const {
    episodes,
    name,
    overview,
  ...
  const guestStars = getUniqueItems(episodes.map((episode) => episode.guest_stars).flat());
